export class CONSTANTS {
    // 插件信息
    public static readonly PLUGIN_NAME: string = "inbox-light";
    public static readonly PLUGIN_NAME_ZH: string = "收件箱简化";

    // 插件设置
    public static readonly SETTING_STORAGE: string = "menu_config";
    public static readonly SETTING_STORAGE_HEIGHT: string = "600px";
    public static readonly SETTING_KEY_INBOXDOCID: string = "inboxDocId";
    public static readonly SETTING_KEY_DELOPCONFIRM: string = "delOpConfirm";
    public static readonly SETTING_KEY_DOCTIMEPREFIX: string = "docTimePrefix";
    public static readonly SETTING_KEY_DELAFTERREFRESH: string = "delAfterRefresh";
    public static readonly SETTING_KEY_REPLACEBUILTIN: string = "replaceBuiltIn";

    // HTML元素样式
    public static readonly STYLE_DISABLED_OPACITY: string = "0.5";
}

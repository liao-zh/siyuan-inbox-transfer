...
- 多选：单击勾选框切换单个选中；在条目上按住拖动可批量选中划过区间，Ctrl+拖动可批量取消划过区间的选中（按住 Ctrl 时按下即生效）；多次拖动可叠加选择，亦可配合全选按钮
- 排序：面板支持按收集箱收集时间排序（新到旧/旧到新）或按名称排序，未记录到收集时间的文档按创建时间参与排序；排序方式会持久化，并在设置面板与面板下拉间双向同步
...
- 0.6.0：面板工具栏拆为两行、全选显示选中计数、支持排序
- 0.7.0：面板支持拖过即选与Ctrl+拖动取消选中，顶栏增加操作提示
- 0.7.2：请求失败不再弹出右上角错误提示（改为日志），并补上各处的空值兜底
